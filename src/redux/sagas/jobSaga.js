...
function* deactivateJob(action) {
    ...
    try {
        yield axios.put(`${apiUrl}/api/jobs/deactivate/${action.payload}`)
...
function* fetchJobStages(action) {
    let allJobStages = yield axios.get(`${apiUrl}/api/jobs/stages`)
    let currentStages = []
...
function* fetchJobTasks(action){
    let allJobTasks = yield axios.get(`${apiUrl}/api/jobs/tasks`)
    let currentTasks = []
...
function* saveJobUpdates(action){
    try{
        ...
        yield axios.delete(`${apiUrl}/api/jobs/stages/${action.payload.job.job_id}`)
        //send stage data
        yield Object.entries(action.payload.stages).map(stage => {
            axios.post(`${apiUrl}/api/jobs/stages`, { stage: stage, job_id: action.payload.job.job_id })
        })
        //delete tasks associated with job before adding all from redux
        yield axios.delete(`${apiUrl}api/jobs/tasks/${action.payload.job.job_id}`)
        // send task data
        yield Object.entries(action.payload.tasks).map(task => {
            if (task.due_date) {
                axios.post(`${apiUrl}/api/jobs/tasks`, { task: task, job_id: action.payload.job.job_id })
            }
